	...
	if spriteKeyboardBack:isPointInside(x, y) then
		...
		for i, key in ipairs(keys) do
			if key:isPointInside(x, y) then
				...
				-- say about recalc
				if pressedKey == spriteKeyEnter and (keyboardParam == "width" or keyboardParam == "stick" or keyboardParam == "diam" or keyboardParam == "offset") and tonumber(keyboardValue) ~= tonumber(formatNumber(balance:getFloatParam(keyboardParam))) and balance:getIntParam("state") == STATE_IDLE and balance:getIntParam("result") == RESULT_SUCCESS then
